...
export async function sendBankTransferPendingEmail({
  to, guestName, bookingNumber, rooms, checkIn, checkOut, totalAmount,
}: {
  to: string; guestName: string; bookingNumber: string;
  rooms: string; checkIn: string; checkOut: string; totalAmount: number;
}) {
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#333">
  ...
  <div style="background:#f9f9f9;padding:28px;border:1px solid #e0e0e0">
    ...
    <p>Thank you for choosing White Vintage Bungalow. Your reservation is pending bank transfer confirmation.</p>
    ...
    <div style="background:#e8f5e9;border-left:4px solid #2d6a4f;padding:16px;border-radius:4px;font-size:14px">
      ...
      <p style="margin:8px 0 4px"><strong>Payment Reference:</strong>
        <span style="color:#2d6a4f;font-weight:bold"> ${bookingNumber}</span></p>
    </div>

    <p style="margin-top:24px">Once transferred, please <strong>upload your receipt</strong> so we can verify and confirm your booking:</p>
    ...
    <p>Your booking will be confirmed within <strong>2–4 hours</strong> after receipt verification.</p>
...
export async function sendBookingConfirmedEmail({
  to, guestName, bookingNumber, rooms, checkIn, checkOut,
}: {
  to: string; guestName: string; bookingNumber: string;
  rooms: string; checkIn: string; checkOut: string;
}) {
  await sendEmail({
    to,
